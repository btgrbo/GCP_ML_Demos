-- Creating a View from the black-friday train dataset

CREATE VIEW `bt-int-ml-specialization.demo2.vw-black-friday-train` AS
WITH transformation_cols
AS (
  SELECT *,

    -- Filling the NULL values of product category 2 and 3 with valid values
    COALESCE(Product_Category_2, 1) AS Product_Category_2_new,
    COALESCE(Product_Category_3, 2) AS Product_Category_3_new,

    -- Updating the temporary column based on '0' and '1' values
    CASE
...
-- Selcting columns needed for the training dataset
SELECT
